...
> **company-os está instalado globalmente.** Use `company-os` diretamente em qualquer terminal.
...
- Inicia a partir do **projeto atual** (diretório corrente do IDE).
...
```bash
# Escanear o projeto atual (diretório corrente)
company-os scan

# Escanear um projeto específico sem precisar de cd
company-os scan --path /caminho/para/meu-projeto
```

- Atualiza o arquivo `.company-os/context.json` dentro do projeto escaneado.
- Use `--path` para escanear qualquer projeto sem precisar mudar de diretório.
